	...
	: CDialog(CHumidityDlg::IDD, pParent)
{
	...
	AddScreenProgramHumidityArea = (pAddScreenProgramHumidityArea)GetProcAddress(hInst,"AddScreenProgramHumidityArea");
}
...
BEGIN_MESSAGE_MAP(CHumidityDlg, CDialog)
	ON_BN_CLICKED(IDC_BUTTON1, &CHumidityDlg::OnBnClickedButton1)
END_MESSAGE_MAP()
...
void CHumidityDlg::OnBnClickedButton1()	//添加湿度区域
{
	// TODO: 在此添加控件通知处理程序代码
	CString strScreenNo;
	...
	int nResult;
            nResult = AddScreenProgramHumidityArea(_ttoi(strScreenNo), _ttoi(strCurProgramOrd)
